...
class RolePermissionControllerIntegrationTest extends IntegrationTestSuite
{
  ...
  class WhenAccess
  {
    ...
    void givenRolePermissions_whenFindAll_thenStatus200AndReturnFullList() throws Exception
    {
      ...
      MvcResult mvcResult = mockMvc
              .perform(get(uri, savedRoleId).contentType(MediaType.APPLICATION_JSON_UTF8))
              .andExpect(status().isOk())
              .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8))
              .andExpect(jsonPath("$", notNullValue()))
              .andReturn();
      String contentAsString = mvcResult.getResponse().getContentAsString();
      ApiResponsePage<Permission> apiResponsePage = objectMapper.readValue(contentAsString, new TypeReference<ApiResponsePage<Permission>>() {});
      ...
      assertThat(savedPermissions.stream()
                    .allMatch(savePermission -> apiResponsePage.stream()
                            .anyMatch(permission -> permission.getId() != null && permission.getId().equals(savePermission.getId()))))
          .isTrue();
    }
  }
  ...
  class WhenAssign
  {
    ...
    void givenRoleAndPermission_whenAssign_thenStatus200AndReturnFullList() throws Exception
    {
      ...
      // Act / Assert
      MvcResult mvcResult = mockMvc
              .perform(post(uri, savedRole.getId(), savedPermission.getId()).contentType(MediaType.APPLICATION_JSON_UTF8))
    ...
    void givenUnknownPermission_whenAssign_thenStatus404() throws Exception
    {
      ...
      // Act / Assert
      mockMvc
          .perform(post(uri, savedRole.getId(), unknownPermissionId).contentType(APPLICATION_JSON_UTF8))
    ...
    void givenUnknownRole_whenAssign_thenStatus404() throws Exception
    {
      ...
      // Act / Assert
      mockMvc
          .perform(post(uri, unknownRoleId, savedPermission.getId()).contentType(APPLICATION_JSON_UTF8))
          .andExpect(status().isBadRequest())
          .andExpect(jsonPath("$", notNullValue()))
  ...
  class WhenUnassign
  {
    ...
    void givenRoleAndPermission_whenUnassign_thenStatus204() throws Exception
    {
      ...
      // Act / Assert
      mockMvc.perform(delete(uri, savedRole.getId(), savedPermission.getId()).contentType(MediaType.APPLICATION_JSON_UTF8)).andExpect(status().isNoContent());
    }
    ...
    void givenUnknownPermission_whenUnassign_thenStatus404() throws Exception
    {
      ...
      // Act / Assert
      mockMvc
          .perform(delete(uri, savedRole.getId(), unknownPermissionId).contentType(APPLICATION_JSON_UTF8))
          .andExpect(status().isBadRequest())
          .andExpect(jsonPath("$", notNullValue()))
          .andExpect(jsonPath("$", is(String.format("Could not find a permission by the specified id [%s]!", unknownPermissionId))));
    }
    ...
    void givenUnknownRole_whenUnassign_thenStatus404() throws Exception
    {
      ...
      // Act / Assert
      mockMvc
          .perform(delete(uri, unknownRoleId, savedPermission.getId()).contentType(APPLICATION_JSON_UTF8))
          .andExpect(status().isBadRequest())
          .andExpect(jsonPath("$", notNullValue()))
          .andExpect(jsonPath("$", is(String.format("Could not find a role by the specified id [%s]!", unknownRoleId))));
    }
